...
public class ClienteDao extends ConnectionFactory {
    ...
    public void inserir (Cliente  cli) throws SQLException{
        ...
        +"(nomecli, endcli, "
         +"cepcli, comcli, endcli, baicli, celcli)"
          +"value (?,?,?,?,?,?)";
        
      try (PreparedStatement st = this.con.prepareStatement(sql)) {
             st.setString(1, cli.getNomcli());
             st.setString(1, cli.getComcli());
             st.setString(1, cli.getBaicli());
             st.setString(1, cli.getEndcli());
             st.setInt(5, cli.getCelcli());
             st.setInt(5, cli.getCepcli());
             st.setInt(5, cli.getCodcli());
